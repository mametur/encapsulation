...
const obj2ZeroRemainders = obj2.getZeroRemainders;
...
const obj2ZeroRemainders2 = obj2.getZeroRemainders;
...
function deepCompare(actual, expect) {
	return (
		...
		(Object(actual) === actual &&
			...
			/* compare arrays */ ((Array.isArray(actual) && Array.isArray(expect) && actual.length === expect.length && expect.every((expect, index) => deepCompare(actual[index], expect))) ||
				/* compare objects */ (Object.keys(actual).length === Object.keys(expect).length && Object.keys(expect).every((key) => deepCompare(actual[key], expect[key])))))
	);
}
